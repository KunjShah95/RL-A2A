...
try:
    ...
    # --- OpenAI SDK integration for server-side LLM features ---
    openai_api_key = os.environ.get("OPENAI_API_KEY")
    if not openai_api_key:
        raise RuntimeError(
            "OpenAI API key not found. Set the OPENAI_API_KEY environment variable for server-side LLM features."
        )
    openai_client = OpenAI(api_key=openai_api_key)
except ImportError as e:
    print(f"ERROR: Missing dependency: {e}")
    print("Please install required packages with:")
    print("pip install fastapi uvicorn websockets msgpack numpy pydantic openai")
    import sys
...
def initialize_q_table(states, actions):
    client = OpenAI()
    prompt = f"Generate initial Q-values for {len(states)} states and {len(actions)} actions."
    response = client.chat.completions.create(
        model="gpt-4", messages=[{"role": "user", "content": prompt}]
    )
    # Parse response to initialize Q-table
    return numpy.array(...)  # Convert response to
...
def update_q_model(
    agent_id: str, state: str, action: str, reward: float, next_state: str
):
...
def queue_message(agent_id: str, message: dict):
    ...
    print(
        f"[Queue] Message for {agent_id}: {message['message']} ({len(queue)} in queue)"
    )
...
async def process_feedback(feedback: Feedback):
    ...
    matching_sessions = [
        s for s in agent_sessions.values() if s["agent_id"] == agent_id
    ]
    ...
    if matching_sessions:
        ...
        if session["last_action"] and session["last_state"]:
            ...
            current_state = (
                feedback.context.get("emotion", "neutral")
                if feedback.context
                else "neutral"
            )
        ...
        while True:
            ...
            if queue:
                ...
                await websocket.send_bytes(
                    msgpack.packb(action.model_dump(), use_bin_type=True)
                )
...
@app.get("/visualize")
def visualize():
    """Visualize Q-tables and agent sessions as JSON."""
    q_tables = {agent_id: model["q_table"].tolist() for agent_id, model in agent_models.items()}
    sessions = {sid: {k: v for k, v in s.items() if k != "q_table"} for sid, s in agent_sessions.items()}
    return JSONResponse({
        "q_tables": q_tables,
...
def visualize_multi():
    """Visualize all agent Q-tables and session info for multiple agents as JSON."""
    q_tables = {agent_id: model["q_table"].tolist() for agent_id, model in agent_models.items()}
    sessions = {sid: {k: v for k, v in s.items() if k != "q_table"} for sid, s in agent_sessions.items()}
    return JSONResponse({
        "q_tables": q_tables,
